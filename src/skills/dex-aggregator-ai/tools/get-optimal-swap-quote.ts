...
export async function getOptimalSwapQuote(params: SwapQuoteParams): Promise<SwapQuote> {
  ...
  const enrichedRoutes: SwapRoute[] = await Promise.all(
    rawQuotes.map(async (quote: unknown) => {
      ...
      const quoteAmount = typeof quoteData.toTokenAmount === 'string' ? quoteData.toTokenAmount : '0';
      const quotePriceImpact = typeof quoteData.priceImpact === 'string' ? quoteData.priceImpact : '0';
      const quoteGasEstimate = typeof quoteData.gasEstimate === 'string' ? quoteData.gasEstimate : '150000';
      const quoteRouterType = typeof quoteData.routerType === 'string' ? quoteData.routerType : 'SINGLE';
      const quoteEstimatedTime = typeof quoteData.estimatedTime === 'string' ? quoteData.estimatedTime : '< 30s';
      const quoteProvider =
      ...
      const meta: RouteMetadata = {
        fromToken, toToken, fromTokenAddress: fromAddress,
        toTokenAddress: toAddress, amount, rawAmount, chain, chainId, expiresAt,
      };
  ...
  return {
    ...
    simulationRecommended:
      recommendedRoute.priceImpact > 0.3 || !recommendedRoute.mevProtected,
  ...
  try {
    ...
    const gasPriceValue =
      gasData && typeof gasData.gasPrice === 'string'
        ...
        : undefined;
    const gwei = gasPriceValue ? parseInt(gasPriceValue, 10) / 1e9 : FALLBACK_GAS_GWEI[chain] ?? 20;
...
export { getRoute as getStoredRoute, getRoute as getStoredRouteWithTTL, getRouteMeta as getRouteMetadata };
